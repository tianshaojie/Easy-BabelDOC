...
async def run_translation(task_id: str, config):
    ...
    try:
        async for event in high_level.async_translate(config):
            ...
            if task_id in active_translations:
                if event["type"] == "progress_update":
                    ...
                    # 确保错误信息是字符串
                    error_msg = str(error_obj) if error_obj else "未知错误"
                    active_translations[task_id].update({
                        "status": "error",
                        "error": error_msg,
                        "end_time": datetime.now().isoformat()
        ...
        # 循环结束后，检查任务是否仍在运行状态（可能是异常退出）
        if task_id in active_translations and active_translations[task_id].get("status") == "running":
            # 尝试从任务数据中获取错误信息
            task_error = active_translations[task_id].get("error")
            
            # 根据任务的stage判断可能的错误原因
            stage = active_translations[task_id].get("stage", "")
            if "Parse Paragraphs" in stage or "段落" in stage:
                error_msg = "文档解析失败：无法从PDF中提取段落。可能是文档格式不支持或内容为纯图片。"
            elif task_error and isinstance(task_error, str) and task_error.strip():
                error_msg = task_error
            else:
                error_msg = "翻译过程异常结束，请检查文档格式是否正确。"
        ...
        # 特殊处理BabelDOC生成的文件错误
        if "InputFileGeneratedByBabelDOCError" in str(type(e)) or "generated by BabelDOC" in error_msg:
            error_msg = "此文件已经是翻译后的文档，无法重复翻译。请上传原始文档。"
